...
export function getCityOptions(provinceCode?: string | null): AddressOption[] {
    ...
    if (cleanProv === NCR_PROVINCE_CODE || cleanProv === NCR_SHORT_CODE || cleanProv === 'NCR') {
        return NCR_CITIES;
    }

    const directMuncities = listMuncities(cleanProv);
    const hucCodes = PROVINCE_TO_HUC_MAP[cleanProv] || [];
...
export function getBarangayOptions(cityCode?: string | null): AddressOption[] {
    ...
    if (cleanCity === '80600' || cleanCity === '806' || cleanCity === '1380600000') {
    ...
    const barangays = listBarangays(cleanCity);
    return barangays
        .map(b => ({ value: b.brgyCode, label: b.brgyName.trim() }))
        .sort((a, b) => a.label.localeCompare(b.label, undefined, { numeric: true }));
}
...
export function resolveProvinceName(provinceCode?: string | null): string {
    ...
    if (clean === NCR_PROVINCE_CODE || clean === NCR_SHORT_CODE || clean === 'NCR' || clean.startsWith('138')) {
        return 'Metro Manila';
    }
...
export function resolveCityName(cityCode?: string | null): string {
    ...
    const ncrMatch = NCR_CITIES.find(c => c.value === clean);
