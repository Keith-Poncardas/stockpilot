...
export function AddressInfo({ control, setValue }: AddressInfoProps) {
    ...
    // Handle province change: reset city and barangay only if province actually changed
    const handleProvinceChange = (newProvinceCode: string, onChange: (val: string) => void) => {
        if (newProvinceCode === selectedProvinceCode) return;
        onChange(newProvinceCode);
        setValue?.('cityCode', '', { shouldValidate: false, shouldDirty: true });
        setValue?.('barangayCode', '', { shouldValidate: false, shouldDirty: true });
    };

    // Handle city change: reset barangay only if city actually changed
    const handleCityChange = (newCityCode: string, onChange: (val: string) => void) => {
        if (newCityCode === selectedCityCode) return;
